  ...
  describe('PUT /booking/:bookingId', () => {
    ...
    describe('when token is valid', () => {
      it('should respond with status 403 when user has no booking', async () => {
        ...
        const result = await server.put('/booking/1').set('Authorization', `Bearer ${token}`).send({
          roomId: room.id,
        });
      ...
      it('should respond with status 404 when roomId doesnt exist', async () => {
        ...
        const result = await server.put(`/booking/${roomIdNumber}}`).set('Authorization', `Bearer ${token}`).send({
          roomId: roomIdNumber,
        });
